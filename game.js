...
class Actor{
	...
	isIntersect(newActor){
		if(!(newActor instanceof Actor)){
			...
			let l = newActor.left;
			let r = newActor.right;
			let t = newActor.top;
			let b = newActor.bottom;	
			let tl = this.left;
			let tr = this.right;
			let tt = this.top;
			let tb = this.bottom;
...
class Coin extends Actor{
	constructor(pos = new Vector(0,0)){
		...
		this.tmp = pos.plus(new Vector(0.2, 0.1));
	}
	...
	getNextPosition(time = 1){
		...
		let a = this.tmp;
		let t = a.plus(this.getSpringVector());
		return t;
	}
	act(time = 1){
...
let d = loadLevels().then(success, err);
//const schemas = [
//  [
//    '         ',
//    '         ',
